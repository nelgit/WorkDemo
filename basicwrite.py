...
def format_file():
    ...
    for root, dirs, files in os.walk(working_dir):
        ...
        for filename in files:  # looks thru every file
            ...
            print(filename)
            print(files)
            print(''.join([os.getcwd(),'/',filename]))
            test = ''.join([os.getcwd(),'/',filename])
            print (root)
            print (dirs)
            print(os.path.isfile(test))
            print(os.path.isfile(''.join([working_dir,'/',filename])))
            ...
            if os.path.isfile(''.join([working_dir,'/',filename])):  # added to fix iteration on directory  #must BE ABSOLUTE PATH
                ...
                if current_path != wildcardhandler.fnmatch(skip_copy_directory, '*'):  # try and prevent any logic from executing in directory and sub/files not 100% successful
                    ...
                    # Make files in new directory, Remove and remake if already there(# not the solution)
                    if exist_dircheck:
                        if exist_filecheck:
